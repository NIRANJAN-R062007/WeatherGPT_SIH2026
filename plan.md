...
> "WeatherGPT turns IMD's authoritative data into a conversation — in English, Hindi, Tamil, Telugu and Marathi, over app, WhatsApp and a plain phone call — with zero hallucinated numbers, because the LLM routes queries and the data answers them."
...
> Turning `"Weather Code": "95", "Wind Direction": "230"` into *"இடியுடன் கூடிய மழை, தென்மேற்கு காற்று"*. Build the decoder module early. Decode to **canonical English terms / stable keys** ("thunderstorm with rain", "southwesterly"), not straight to one language — the translation layer renders those into all five. One decoder, five outputs; never five decoders.
...
5. Warning colour-code rendering (Red/Orange/Yellow/Green) with official category text, rendered in all five languages via the `data/i18n/` glossary.
...
| Week | Deliverable |
|---|---|
| W1 | Verify every IMD endpoint by hand (curl each one, save sample JSON). Lock architecture. Repo + Docker Compose skeleton. Decoder tables (canonical keys) + `data/i18n/` skeleton done. Line up a native Telugu and Marathi speaker for QA. |
| W1 | Thin slice: `GET /ask` → "what's the weather in Chennai" → real IMD data → English answer with provenance. |
| W2 | Flutter chat UI, all five languages as text. District warnings with colour codes + translated category text. Basic voice (Tamil) via Bhashini for the demo hook. |
...
2. **(0:45) Grounding** — Tap "sources." Show it pulled IMD District Nowcast issued at a real timestamp. Say the line: **"Our LLM cannot produce a number. It routes; IMD answers."** Then switch the same answer to Hindi and Marathi on screen — one tap, no reload — to show language is a config flag, not a hardcode.
