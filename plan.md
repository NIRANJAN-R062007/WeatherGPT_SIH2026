...
- ~~Telugu-script + Devanagari font bundles and layout check in the UI.~~ ✅ done (2026-09-26, picked up by **Mahesh** — originally **Chelsea**) — `web/` self-hosts Noto Sans Devanagari/Telugu/Tamil (Tamil added too: same gap, same live strings) as `unicode-range`-scoped `@font-face` blocks under the existing `font-family: 'Inter'` name, so every existing hi/te/ta string (Settings' language pills, the "Ask in ..." copy on Home/Chat) renders in a real bundled face instead of silently falling back to whatever Indic font the visitor's OS happened to have. Verified live in a browser: all three faces actually `loaded` (not just registered), `overflow: visible` with comfortable padding everywhere this text renders, so no clipping at Devanagari/Telugu's taller vowel-sign/conjunct extents.
